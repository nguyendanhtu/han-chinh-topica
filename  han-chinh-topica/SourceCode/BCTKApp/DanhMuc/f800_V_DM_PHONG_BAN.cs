...
,TEN = 3
		...
		DS_V_DM_PHONG_BAN m_ds = new DS_V_DM_PHONG_BAN();
		US_V_DM_PHONG_BAN m_us = new US_V_DM_PHONG_BAN();
		#endregion
		...
		private ITransferDataRow get_trans_object(C1.Win.C1FlexGrid.C1FlexGrid i_fg){
			...
			v_htb.Add(V_DM_PHONG_BAN.TEN_PHONG_BAN, e_col_Number.TEN_PHONG_BAN);
			v_htb.Add(V_DM_PHONG_BAN.MA_PHONG_BAN, e_col_Number.MA_PHONG_BAN);
			v_htb.Add(V_DM_PHONG_BAN.TEN, e_col_Number.TEN);
									
			ITransferDataRow v_obj_trans = new CC1TransferDataRow(i_fg,v_htb,m_ds.V_DM_PHONG_BAN.NewRow());
			return v_obj_trans;			
		}
		private void load_data_2_grid(){						
			m_ds = new DS_V_DM_PHONG_BAN();			
			m_us.FillDataset(m_ds);
			m_fg.Redraw = false;
			CGridUtils.Dataset2C1Grid(m_ds, m_fg, m_obj_trans);
			m_fg.Redraw = true;
		}
		private void grid2us_object(US_V_DM_PHONG_BAN i_us
			, int i_grid_row) {
			DataRow v_dr;
		...
		private void us_object2grid(US_V_DM_PHONG_BAN i_us
			, int i_grid_row) {
		...
		private void insert_v_dm_phong_ban(){			
			f801_DM_PHONG_BAN_DE v_fDE = new  f801_DM_PHONG_BAN_DE();								
			v_fDE.display_for_insert();
			load_data_2_grid();
		}

		private void update_v_dm_phong_ban(){			
		...
		private void delete_v_dm_phong_ban(){
			if (!CGridUtils.IsThere_Any_NonFixed_Row(m_fg)) return;
			if (!CGridUtils.isValid_NonFixed_RowIndex(m_fg, m_fg.Row)) return;
			if (BaseMessages.askUser_DataCouldBeDeleted(8) != BaseMessages.IsDataCouldBeDeleted.CouldBeDeleted)  return;
			US_V_DM_PHONG_BAN v_us = new US_V_DM_PHONG_BAN();
		...
		private void view_v_dm_phong_ban(){			
			if (!CGridUtils.IsThere_Any_NonFixed_Row(m_fg)) return;
			if (!CGridUtils.isValid_NonFixed_RowIndex(m_fg, m_fg.Row)) return;
			grid2us_object(m_us, m_fg.Row);
		//	f800_V_DM_PHONG_BAN_DE v_fDE = new f800_V_DM_PHONG_BAN_DE();			
		//	v_fDE.display(m_us);
		}
