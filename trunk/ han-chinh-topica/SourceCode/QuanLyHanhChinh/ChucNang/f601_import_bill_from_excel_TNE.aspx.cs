...
public partial class ChucNang_f601_import_bill_from_excel_TNE : System.Web.UI.Page
{
    ...
    private void excel_file_to_grid(GridView ip_grv, int ip_i_start_row, string ip_str_file_dir)
    {
        ...
        for (int i = 0; i < v_i_row_count; i++)
        {
            ...
            v_dr["STT"] = CIPConvert.ToDecimal(v_ds_result.Tables[0].Rows[i][0]); //stt
            v_dr["NOI_DUNG"] = ""; // nội dung
            v_dr["NOI_NHAN"] = v_ds_result.Tables[0].Rows[i][4].ToString(); // nơi nhận
            v_dr["NGUOI_NHAN"] = v_ds_result.Tables[0].Rows[i][1].ToString() + " " + v_ds_result.Tables[0].Rows[i][2].ToString(); //người nhận
